    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li><a href="${pageContext.request.contextPath}/pages/back/cashier_CheckIn.jsp">前台收银管理</a></li>
        ...
        <section class="content">
                <div class="box box-primary"    >
                    ...
                    <div class="box-body">
                        <!-- 数据表格 -->
                        <div class="table-box">
                            <form action="${pageContext.request.contextPath}/order/findMemberByRealNameAndIdCard?chose=2" method="post" >
                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                姓名：<input type="text" name="realName"/>
                                &nbsp;&nbsp;&nbsp;
                                身份证：<input type="text" name="identificationNum"/>
                                &nbsp;&nbsp;&nbsp;
                                <input type="submit" value="搜索"/>
                                <input type="button" onclick="location.href='${pageContext.request.contextPath}/order/findReserveByPage?page=1&size=5'" value="查询所有"/>
                            </form>
                            <%--<!--工具栏-->--%>
                            <%--<div class="pull-left">--%>
...
<%--                                        <td><input name="ids" type="checkbox"></td>--%>
                                        <td>${(pageInfo.pageNum-1)*pageInfo.pageSize+state.count}</td>
                                        <td>${order.house.houseName}</td>
                                        <td>${order.house.houseNum}</td>
                                        <td>${order.orderPrice}</td>
                                        <td>${order.realName}</td>
                                        <td>${order.identificationNum}</td>
                                        ...
                                        <td class="text-center zjxg">
                                            <button type="button" class="btn bg-olive btn-xs" onclick='location.href="${pageContext.request.contextPath}/order/updateToCompleteById?oid=${order.oid}"'>办理入住</button>
                                        </td>
                                    </tr>
                                </c:forEach>
                                </tbody>
                            ...
                            <ul class="pagination">
                                ...
                                <c:forEach begin="1" var="i" end="${pageInfo.pages}">
                                    ...
                                    <c:if test="${pageInfo.pageNum == i}">
                                        <li class="active"><a href="${pageContext.request.contextPath}/order/findReserveByPage?page=${i}&size=${pageInfo.pageSize}">${i}</a></li>
                                    </c:if>
                                    <c:if test="${pageInfo.pageNum != i}">
                                        <li><a href="${pageContext.request.contextPath}/order/findReserveByPage?page=${i}&size=${pageInfo.pageSize}">${i}</a></li>
                                    </c:if>
                                </c:forEach>
...
<script>
    ...
    <%--    $.post("${pageContext.request.contextPath}/order/findReserveByPage",{"page":1,"size":pageSize},function (data) {--%>

    <%--    },"json")--%>
    <%--}--%>

    <%--function beforePage() {--%>
    <%--    $.post("${pageContext.request.contextPath}/order/findReserveByPage",{"page":${sessionScope.pageNum}-1,"size":pageSize},function (data) {--%>
    ...
    <%--    var pageSize = $("#change").val();--%>
    <%--    $.post("${pageContext.request.contextPath}/order/findReserveByPage",{"page":${sessionScope.pageNum},"size":pageSize},function (data) {--%>
